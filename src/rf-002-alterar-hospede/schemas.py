...
class HospedeUpdateSchema(BaseModel):
    """
    Payload de entrada para atualização cadastral de hóspedes (RF-002).
    O CPF é estritamente omitido deste DTO para garantir sua imutabilidade legal.
    """
    nome: str = Field(..., min_length=3, max_length=150, description="Nome completo do hóspede")
    email: EmailStr = Field(..., description="Novo endereço de e-mail válido (RFC 5322)")
    telefone: str = Field(..., min_length=10, max_length=20, description="Telefone atualizado com DDD")
    observacoes: Optional[str] = Field(None, max_length=1000, description="Observações ou notas operacionais atualizadas")

    @field_validator("nome")
    @classmethod
    def sanitizar_nome(cls, v: str) -> str:
        """Sanitização anti-XSS do nome."""
        texto_limpo = html.escape(v.strip())
        if len(texto_limpo) < 3:
            raise ValueError("O nome do hóspede deve conter no mínimo 3 caracteres válidos.")
        return texto_limpo
    ...
    @classmethod
    def sanitizar_observacoes(cls, v: Optional[str]) -> Optional[str]:
        """Sanitização anti-XSS de observações."""
        if v:
            return html.escape(v.strip())
        return v
    ...
    @classmethod
    def sanitizar_telefone(cls, v: str) -> str:
        """Validação do formato do telefone com DDD."""
        numeros = re.sub(r"\D", "", v)
        if len(numeros) < 10 or len(numeros) > 15:
            raise ValueError("O telefone deve conter DDD e entre 10 e 15 dígitos numéricos.")
        return v.strip()
...
class HospedeStatusSchema(BaseModel):
    """
    Payload de entrada para alternância de status ativo/inativo (RF-002 - Soft Delete).
    """
    is_ativo: int = Field(..., ge=0, le=1, description="Status do cadastro: 1 = Ativo, 0 = Inativo (Soft Delete)")
