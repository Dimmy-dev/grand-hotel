...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // --------------------------------------------------------------------------
  // 9. GESTÃO DE HÓSPEDES: BUSCA PREDITIVA, FILTROS, EDIÇÃO E STATUS (RF-002)
  // --------------------------------------------------------------------------

  // Elementos do RF-002
  const inputBusca = document.getElementById("input-busca-hospede");
  const filtroTodos = document.getElementById("filtro-todos");
  const filtroAtivos = document.getElementById("filtro-ativos");
  const filtroInativos = document.getElementById("filtro-inativos");

  // Elementos do Modal de Edição
  const modalEdicao = document.getElementById("modal-edicao-hospede");
  const formEdicao = document.getElementById("form-edicao-hospede");
  const editUuid = document.getElementById("edit-hospede-uuid");
  const editCpf = document.getElementById("edit-hospede-cpf");
  const editNome = document.getElementById("edit-hospede-nome");
  const editEmail = document.getElementById("edit-hospede-email");
  const editTelefone = document.getElementById("edit-hospede-telefone");
  const editObservacoes = document.getElementById("edit-hospede-observacoes");
  ...
  const textBtnEdit = document.getElementById("text-btn-edit");
  const erroEditNome = document.getElementById("erro-edit-nome");
  const erroEditEmail = document.getElementById("erro-edit-email");
  const erroEditTelefone = document.getElementById("erro-edit-telefone");

  let statusFiltroAtual = "todos";
  let debounceBuscaTimeout = null;

  // Máscara dinâmica de telefone no modal de edição
  ...
  // Debounce de 300ms no input de busca para evitar sobrecarga na API
  ...
  // Alternância das Pills de Filtragem
  [filtroTodos, filtroAtivos, filtroInativos].forEach(btn => {
    btn.addEventListener("click", () => {
      [filtroTodos, filtroAtivos, filtroInativos].forEach(b => b.classList.remove("active"));
      btn.classList.add("active");
      statusFiltroAtual = btn.getAttribute("data-status");
      carregarHospedes();
    });
  });
  ...
  async function carregarHospedes() {
    try {
      ...
      if (statusFiltroAtual !== "todos") params.append("status_filtro", statusFiltroAtual);

      const url = "/api/v1/hospedes" + (params.toString() ? "?" + params.toString() : "");
  ...
  function carregarFallbackLocalStorage() {
    ...
    // Aplica filtro local em contingência
    let filtrados = [...hospedesLocais];
    const termo = inputBusca.value.trim().toLowerCase();
    if (termo) {
      filtrados = filtrados.filter(h => 
        (h.nome && h.nome.toLowerCase().includes(termo)) ||
        (h.cpf && h.cpf.includes(termo)) ||
        (h.email && h.email.toLowerCase().includes(termo))
      );
    }
    if (statusFiltroAtual === "ativos") {
      filtrados = filtrados.filter(h => h.is_ativo !== 0);
    } else if (statusFiltroAtual === "inativos") {
      filtrados = filtrados.filter(h => h.is_ativo === 0);
    }
    renderizarTabelaHospedes(filtrados);
  }
  ...
  function renderizarTabelaHospedes(lista) {
      ...
      tabelaHospedesBody.innerHTML = `
        <tr>
          <td colspan="6" style="text-align: center; color: var(--text-muted); padding: 1.5rem;">
            Nenhum hóspede localizado para os filtros informados.
          </td>
        </tr>
      `;
      return;
    ...
    tabelaHospedesBody.innerHTML = lista.map(h => {
      ...
      const isAtivo = h.is_ativo !== 0;
      const badgeStatus = isAtivo 
        ? '<span class="badge-status badge-status-ativo"><span style="display:inline-block;width:6px;height:6px;border-radius:50%;background:#16a34a;"></span> Ativo</span>'
        : '<span class="badge-status badge-status-inativo"><span style="display:inline-block;width:6px;height:6px;border-radius:50%;background:#94a3b8;"></span> Inativo</span>';

      const btnToggleTexto = isAtivo ? "Inativar" : "Reativar";
        ...
        <tr data-uuid="${h.uuid}">
          ...
          <td style="text-align: right;">
            <div class="action-buttons-group" style="justify-content: flex-end;">
              <button class="btn-action btn-action-edit" onclick="window.abrirModalEdicao('${h.uuid}')" title="Editar dados cadastrais">
                Editar
              </button>
              <button class="btn-action btn-action-toggle" onclick="window.alternarStatus('${h.uuid}', ${isAtivo ? 1 : 0})" title="${isAtivo ? 'Desativar hóspede (Soft Delete)' : 'Reativar hóspede'}">
                ${btnToggleTexto}
              </button>
            </div>
          </td>
  ...
  window.abrirModalEdicao = async function(uuid) {
    // 1. Busca hóspede na lista local ou via API
    let hospede = hospedesLocais.find(h => h.uuid === uuid);
    if (!hospede) {
      try {
        const resp = await fetch(`/api/v1/hospedes/${uuid}`, { credentials: "include" });
        const json = await resp.json();
        if (resp.ok && json.success) hospede = json.data;
      } catch (e) {
        console.error("Falha ao buscar detalhes do hóspede:", e);
      }
    }

    if (!hospede) {
      alert("Não foi possível carregar os dados deste hóspede.");
      return;
    }

    // 2. Preenche os campos do formulário no modal
    editUuid.value = hospede.uuid;
    editCpf.value = hospede.cpf ? hospede.cpf.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, "$1.$2.$3-$4") : "---";
    editNome.value = hospede.nome || "";
    editEmail.value = hospede.email || "";
    editTelefone.value = hospede.telefone || "";
    editObservacoes.value = hospede.observacoes || "";

    // 3. Limpa erros anteriores
    [erroEditNome, erroEditEmail, erroEditTelefone].forEach(e => e.classList.remove("visible"));
    [editNome, editEmail, editTelefone].forEach(i => i.classList.remove("invalid"));
  ...
  btnFecharModalX.addEventListener("click", fecharModalEdicao);
  btnCancelarModal.addEventListener("click", fecharModalEdicao);

  // Submissão da Edição (PUT /api/v1/hospedes/{uuid})
  formEdicao.addEventListener("submit", async (e) => {
    ...
    const observacoes = editObservacoes.value.trim() || null;

    let valido = true;
    if (nome.length < 3) {
      editNome.classList.add("invalid");
      erroEditNome.classList.add("visible");
      valido = false;
    } else {
      editNome.classList.remove("invalid");
      erroEditNome.classList.remove("visible");
    }

    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(email)) {
      editEmail.classList.add("invalid");
      erroEditEmail.textContent = "Informe um e-mail válido.";
      erroEditEmail.classList.add("visible");
      valido = false;
    } else {
      editEmail.classList.remove("invalid");
      erroEditEmail.classList.remove("visible");
    }

    const telLimpo = telefone.replace(/\D/g, "");
    if (telLimpo.length < 10 || telLimpo.length > 15) {
      editTelefone.classList.add("invalid");
      erroEditTelefone.classList.add("visible");
      valido = false;
    } else {
      editTelefone.classList.remove("invalid");
      erroEditTelefone.classList.remove("visible");
    }

    if (!valido) return;

    // Ativa estado de carregamento do modal
    spinnerEdit.style.display = "inline-block";
    textBtnEdit.textContent = "Salvando...";
    btnSalvarEdicao.disabled = true;

    const payload = { nome, email, telefone, observacoes };

    try {
      const resp = await fetch(`/api/v1/hospedes/${uuid}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify(payload)
      ...
      if (resp.ok && json.success) {
        ...
        editEmail.classList.add("invalid");
        erroEditEmail.textContent = json.error.message || "Este e-mail já pertence a outro hóspede.";
        erroEditEmail.classList.add("visible");
      } else {
        alert(json.error ? json.error.message : "Erro ao salvar alterações.");
      }
    } catch (err) {
      console.warn("Erro de rede na edição. Atualizando localmente:", err);
      // Atualização no localStorage de contingência
      const idx = hospedesLocais.findIndex(h => h.uuid === uuid);
      if (idx !== -1) {
        hospedesLocais[idx] = { ...hospedesLocais[idx], ...payload };
        localStorage.setItem("grand_plaza_hospedes_offline", JSON.stringify(hospedesLocais));
        renderizarTabelaHospedes(hospedesLocais);
      }
      fecharModalEdicao();
      ...
      textBtnEdit.textContent = "Salvar Alterações";
      btnSalvarEdicao.disabled = false;
    }
  });

  // Alternância de Status (Soft Delete / Reativação)
  window.alternarStatus = async function(uuid, statusAtual) {
    const novoStatus = statusAtual === 1 ? 0 : 1;
    const acaoNome = novoStatus === 0 ? "inativar" : "reativar";

    if (!confirm(`Deseja realmente ${acaoNome} o cadastro deste hóspede?`)) {
      return;
    }

    try {
      const resp = await fetch(`/api/v1/hospedes/${uuid}/status`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
      ...
      if (resp.ok && json.success) {
        ...
        alert(json.error ? json.error.message : `Não foi possível ${acaoNome} o hóspede.`);
      }
    } catch (err) {
      console.warn(`Erro de rede ao ${acaoNome}. Atualizando offline:`, err);
      const idx = hospedesLocais.findIndex(h => h.uuid === uuid);
      if (idx !== -1) {
        hospedesLocais[idx].is_ativo = novoStatus;
        localStorage.setItem("grand_plaza_hospedes_offline", JSON.stringify(hospedesLocais));
        renderizarTabelaHospedes(hospedesLocais);
      }
    }
