...
@app.get("/api/v1/hospedes", summary="Listagem e Busca Preditiva de Hóspedes", tags=["Hóspedes"])
def listar_hospedes(
    q: Optional[str] = None,
    ...
    """
    Retorna a lista de hóspedes com suporte a busca preditiva por Nome, CPF ou E-mail
    e filtragem por status (todos, ativos, inativos).
    """
    query = db.query(HospedeModel)
    
    # 1. Filtragem por status operacional
    if status_filtro == "ativos":
        query = query.filter(HospedeModel.is_ativo == 1)
    elif status_filtro == "inativos":
        query = query.filter(HospedeModel.is_ativo == 0)

    # 2. Busca Preditiva (Multi-campo: Nome, CPF ou E-mail)
    if q and q.strip():
        termo_limpo = q.strip()
        termo_like = f"%{termo_limpo}%"
        numeros = re.sub(r"\D", "", termo_limpo)
        
        condicoes = [
            HospedeModel.nome.ilike(termo_like),
            HospedeModel.email.ilike(termo_like)
        ]
        if numeros:
            condicoes.append(HospedeModel.cpf.like(f"%{numeros}%"))
        else:
            condicoes.append(HospedeModel.cpf.like(termo_like))
            
        query = query.filter(or_(*condicoes))

    hospedes = query.order_by(HospedeModel.id.desc()).limit(100).all()
    lista_formatada = [
        {
...
@app.get("/api/v1/hospedes/{hospede_uuid}", summary="Detalhes do Hóspede por UUID", tags=["Hóspedes"])
def obter_hospede(hospede_uuid: str, db: Session = Depends(get_db)):
    """
    Retorna os dados cadastrais detalhados de um hóspede específico através do seu UUID público.
    """
    hospede = db.query(HospedeModel).filter(HospedeModel.uuid_publico == hospede_uuid).first()
    if not hospede:
        return JSONResponse(
            ...
            content={
                "success": False,
                "error": {
                    "code": "HOSPEDE_NOT_FOUND",
                    "message": "Hóspede não localizado para o identificador fornecido."
                }
            }
        )
    return {
        "success": True,
        "data": {
            "uuid": hospede.uuid_publico,
            "nome": hospede.nome,
            "email": hospede.email,
            "cpf": hospede.cpf,
            "telefone": hospede.telefone,
            "data_nascimento": str(hospede.data_nascimento),
            "observacoes": hospede.observacoes,
            "is_ativo": hospede.is_ativo,
            "created_at": hospede.created_at.isoformat() if hospede.created_at else None,
            "updated_at": hospede.updated_at.isoformat() if hospede.updated_at else None
        }
    }


@app.put("/api/v1/hospedes/{hospede_uuid}", summary="Atualização Cadastral de Hóspede", tags=["Hóspedes"])
def atualizar_hospede(
    ...
    """
    Atualiza dados cadastrais de contato do hóspede (nome, e-mail, telefone, observações).
    O CPF permanece estritamente imutável para conformidade legal e auditoria.
    """
    hospede = db.query(HospedeModel).filter(HospedeModel.uuid_publico == hospede_uuid).first()
    if not hospede:
        return JSONResponse(
            ...
            content={
                ...
                "error": {
                    "code": "HOSPEDE_NOT_FOUND",
                    "message": "Hóspede não localizado para atualização."
                }
            }
        )

    # 1. Verifica se o novo e-mail já está sendo utilizado por outro hóspede
    novo_email = payload.email.lower().strip()
    if novo_email != hospede.email:
        hospede_existente = db.query(HospedeModel).filter(
            HospedeModel.email == novo_email,
            HospedeModel.id != hospede.id
        ).first()
        if hospede_existente:
            return JSONResponse(
                ...
                content={
                    ...
                    "error": {
                        "code": "DUPLICATE_EMAIL",
                        "message": "Este endereço de e-mail já pertence a outro hóspede cadastrado.",
                        "field": "email"
                    }
                }
            )

    # 2. Registra snapshot das alterações para a trilha LGPD
    alteracoes = {}
    if hospede.nome != payload.nome:
        alteracoes["nome"] = {"de": hospede.nome, "para": payload.nome}
        hospede.nome = payload.nome
    if hospede.email != novo_email:
        alteracoes["email"] = {"de": hospede.email, "para": novo_email}
        hospede.email = novo_email
    if hospede.telefone != payload.telefone:
        alteracoes["telefone"] = {"de": hospede.telefone, "para": payload.telefone}
        hospede.telefone = payload.telefone
    if hospede.observacoes != payload.observacoes:
        alteracoes["observacoes"] = {"de": hospede.observacoes, "para": payload.observacoes}
        hospede.observacoes = payload.observacoes

    hospede.updated_at = datetime.utcnow()

    try:
        # 3. Grava log de auditoria LGPD (RN-05)
        id_op = getattr(operador, "id", None) if operador else None
        log_audit = AuditLogModel(
            id_operador=id_op,
            acao="ALTERACAO_HOSPEDE",
            tabela_afetada="tb_hospedes",
            registro_id=hospede.id,
            dados_novos=json.dumps({
                "uuid": hospede.uuid_publico,
        ...
        return JSONResponse(
            ...
            content={
                ...
                "error": {
                    "code": "DB_PERSISTENCE_ERROR",
                    "message": f"Erro ao atualizar hóspede no banco de dados: {str(e)}"
                }
            }
        )
...
@app.patch("/api/v1/hospedes/{hospede_uuid}/status", summary="Alternar Status de Hóspede (Soft Delete / Reativação)", tags=["Hóspedes"])
def alternar_status_hospede(
    hospede_uuid: str,
    payload: HospedeStatusSchema,
    request: Request,
    db: Session = Depends(get_db),
    operador: Optional[OperadorModel] = Depends(obter_operador_autenticado)
):
    """
    Ativa ou inativa o cadastro do hóspede de forma lógica (Soft Delete).
    Preserva o histórico referencial e registra evento de auditoria LGPD.
    """
    hospede = db.query(HospedeModel).filter(HospedeModel.uuid_publico == hospede_uuid).first()
    if not hospede:
        return JSONResponse(
            ...
            content={
                ...
                "error": {
                    "code": "HOSPEDE_NOT_FOUND",
                    "message": "Hóspede não localizado."
                }
            }
        )

    status_anterior = hospede.is_ativo
    hospede.is_ativo = payload.is_ativo
    hospede.updated_at = datetime.utcnow()

    acao_audit = "DESATIVACAO_HOSPEDE" if payload.is_ativo == 0 else "REATIVACAO_HOSPEDE"
    ...
    try:
        ...
        return JSONResponse(
            ...
            content={
                ...
                "error": {
                    "code": "DB_PERSISTENCE_ERROR",
                    "message": f"Erro ao alternar status no banco de dados: {str(e)}"
                }
            }
        )
    ...
    return {
        ...
        "data": {
            ...
            "is_ativo": hospede.is_ativo,
            "updated_at": hospede.updated_at.isoformat() if hospede.updated_at else None,
            "message": f"Hóspede {'reativado' if payload.is_ativo == 1 else 'desativado'} com sucesso."
        }
    }
